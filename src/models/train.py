...
def load_feature_data(input_path: Path) -> pd.DataFrame:
    ...
    if not input_path.exists():
        raise FileNotFoundError(
            f"Feature dataset was not found: {input_path}"
        )
...
def validate_training_data(data: pd.DataFrame) -> None:
    ...
    if len(data) < 50:
        raise ValueError(
            "At least 50 rows are required for model training."
        )
...
def prepare_training_data(
    ...
    if prepared[TIMESTAMP_COLUMN].isna().any():
        raise ValueError(
            "Training data contains invalid timestamps."
        )
...
def evaluate_model(
    ...
    positive_probabilities = model.predict_proba(
        split.x_test
    )[:, 1]
...
def print_evaluation(
    ...
    print(
        f"Actual 0         "
        f"{matrix[0][0]:11d}  {matrix[0][1]:11d}"
    )
    print(
        f"Actual 1         "
        f"{matrix[1][0]:11d}  {matrix[1][1]:11d}"
    )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Train a Logistic Regression model for "
            "next-hour Gold price direction."
        )
    )
...
def main() -> None:
    ...
    metadata: dict[str, Any] = {
        ...
        "training_period": {
            "start": (
                split.train_timestamps.min().isoformat()
            ),
            "end": (
                split.train_timestamps.max().isoformat()
            ),
        },
        "testing_period": {
            "start": (
                split.test_timestamps.min().isoformat()
            ),
            "end": (
                split.test_timestamps.max().isoformat()
            ),
        },
        "training_config": {
            key: str(value)
    ...
    print(
        "\nModel saved to: "
        f"{config.model_output_path}"
    )

    print(
        "Metadata saved to: "
        f"{config.metadata_output_path}"
    )
