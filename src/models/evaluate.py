...
def calculate_next_returns(
    ...
    next_returns = (next_close / data[CLOSE_COLUMN]) - 1.0
...
def create_chronological_split(
    ...
    if train_data.empty or test_data.empty:
        raise ValueError(
            "The chronological split produced an empty train or test dataset."
        )
...
def calculate_strategy_results(
    ...
    results["strategy_return"] = results["position"] * results[NEXT_RETURN_COLUMN]

    results["strategy_win"] = results["strategy_return"] > 0
    ...
    strategy_metrics = {
        "number_of_trades": float(len(results)),
        "winning_trades": float(results["strategy_win"].sum()),
        "losing_trades": float((~results["strategy_win"]).sum()),
        "win_rate": float(results["strategy_win"].mean()),
        "cumulative_strategy_return": float(
            results["cumulative_strategy_return"].iloc[-1]
        ),
        "buy_and_hold_return": float(
            results["cumulative_buy_and_hold_return"].iloc[-1]
        ),
        "average_trade_return": float(results["strategy_return"].mean()),
        "best_trade_return": float(results["strategy_return"].max()),
        "worst_trade_return": float(results["strategy_return"].min()),
    }
...
def save_cumulative_return_chart(
    ...
    axis.plot(
        ...
        strategy_results["cumulative_strategy_return"],
        label="Model strategy",
    )

    axis.plot(
        strategy_results[TIMESTAMP_COLUMN],
        strategy_results["cumulative_buy_and_hold_return"],
        label="Buy and hold",
    )
    ...
    axis.set_title("XAUUSD Test-Period Cumulative Returns")
...
def evaluate_model(
    ...
    evaluation_data[NEXT_RETURN_COLUMN] = calculate_next_returns(evaluation_data)
    ...
    train_data, test_data = create_chronological_split(
        evaluation_data,
        test_size=test_size,
    )
    ...
    probabilities_up = model.predict_proba(x_test)[:, 1]

    predictions = (probabilities_up >= threshold).astype(int)
    ...
    strategy_results, strategy_metrics = calculate_strategy_results(
        test_data=test_data,
        predictions=predictions,
    )
    ...
    results: dict[str, Any] = {
        ...
        "dataset": {
            "total_rows": len(evaluation_data),
            "training_rows": len(train_data),
            "testing_rows": len(test_data),
            "purged_boundary_rows": 1,
            "training_period": {
                "start": train_data[TIMESTAMP_COLUMN].min().isoformat(),
                "end": train_data[TIMESTAMP_COLUMN].max().isoformat(),
            },
            "testing_period": {
                "start": test_data[TIMESTAMP_COLUMN].min().isoformat(),
                "end": test_data[TIMESTAMP_COLUMN].max().isoformat(),
            },
        },
        ...
        "limitations": [
            ...
            ("Every model prediction is treated as a long or short trade."),
            (
...
def print_results(
    ...
    print(f"Training rows: {dataset['training_rows']}")
    print(f"Testing rows:  {dataset['testing_rows']}")
    print(f"Purged boundary rows: {dataset['purged_boundary_rows']}")
    ...
    print(f"ROC-AUC:           {classification['roc_auc']:.4f}")

    print("\nNaive strategy metrics")
    print("----------------------")
    print(f"Trades:            {int(strategy['number_of_trades'])}")
    print(f"Win rate:          {strategy['win_rate']:.4f}")
    print(f"Strategy return:   {strategy['cumulative_strategy_return']:.4%}")
    print(f"Buy-and-hold:      {strategy['buy_and_hold_return']:.4%}")
    print(f"Average trade:     {strategy['average_trade_return']:.6%}")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/evaluation_metrics.json"),
    )

    parser.add_argument(
        "--chart",
        type=Path,
        default=Path("artifacts/cumulative_returns.png"),
    )

    parser.add_argument(
        "--trades-output",
        type=Path,
        default=Path("artifacts/test_period_trades.csv"),
    )
...
def main() -> None:
    ...
    print(f"\nEvaluation metrics saved to: {args.output}")
    print(f"Cumulative-return chart saved to: {args.chart}")
    print(f"Test-period trades saved to: {args.trades_output}")
