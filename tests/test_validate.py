    ...
    return pd.DataFrame(
        {
            ...
            "ma_gap": [((index % 11) - 5) / 1_000 for index in range(rows)],
            "volatility_10": [0.001 + (index % 6) / 10_000 for index in range(rows)],
            "candle_body_ratio": [0.1 + (index % 8) / 10 for index in range(rows)],
            "rsi_14": [30.0 + (index % 40) for index in range(rows)],
...
def test_validation_creates_requested_folds() -> None:
    ...
    data = prepare_training_data(create_validation_data())
...
def test_validation_preserves_time_order() -> None:
    ...
    for fold in results:
        training_end = pd.Timestamp(fold["training_period"]["end"])

        testing_start = pd.Timestamp(fold["testing_period"]["start"])
...
def test_calculate_metrics_returns_values() -> None:
    ...
    probabilities = pd.Series([0.2, 0.8, 0.7, 0.9]).to_numpy()
